const ProgressModel = require("../models/ProgressModel");

exports.getStudentProgress = async (req, res) => {
  const student_id = req.user.student_id;
  const academic_year_id = req.user.academic_year_id;

  try {
    // FETCH CORE DATA
    const hours = await ProgressModel.getStudentHours(
      student_id,
      academic_year_id
    );

    const narratives = await ProgressModel.getNarrativeStats(
      student_id,
      academic_year_id
    );

    const dailyLogs = await ProgressModel.getDailyLogStats(
      student_id,
      academic_year_id
    );

    const attendance = await ProgressModel.getAttendanceStats(
      student_id,
      academic_year_id
    );

    if (!hours) {
      return res.json(null);
    }

    const required = Number(hours.required_hours || 0);
    const completed = Number(hours.completed_hours || 0);

    const completionPercent =
      required > 0
        ? Math.min(Math.round((completed / required) * 100), 100)
        : 0;

    const requiredHoursCompleted =
      completed >= required - 0.25;

    const dailyLogsComplete =
      Number(dailyLogs.total || 0) > 0 &&
      Number(dailyLogs.approved || 0) === Number(dailyLogs.total || 0);

    const narrativesApproved =
      Number(narratives.total || 0) > 0 &&
      Number(narratives.approved || 0) === Number(narratives.total || 0);

    const coordinatorVerified =
      requiredHoursCompleted &&
      dailyLogsComplete &&
      narrativesApproved;

    return res.json({
      student_name: hours.student_name,
      department_name: hours.department_name,
      course_name: hours.course_name,
      coordinator_name: hours.coordinator_name,
      company_name: hours.company_name,

      required_hours: required,
      completed_hours: completed,
      remaining_hours: Math.max(required - completed, 0),

      completion_percent: completionPercent,

      attendance,
      dailyLogs,
      narratives,

      checklist: {
        requiredHoursCompleted,
        dailyLogsComplete,
        narrativesApproved,
        coordinatorVerified
      }
    });

  } catch (err) {
    console.error("Progress fetch error:", err);
    return res.status(500).json({
      error: "Failed to fetch progress"
    });
  }
};

exports.getStudentProgressById = async (req, res) => {
  const student_id = Number(req.params.student_id);
  const academic_year_id = req.user.academic_year_id;

  try {
    const hours = await ProgressModel.getStudentHours(
      student_id,
      academic_year_id
    );

    const narratives = await ProgressModel.getNarrativeStats(
      student_id,
      academic_year_id
    );

    const dailyLogs = await ProgressModel.getDailyLogStats(
      student_id,
      academic_year_id
    );

    const attendance = await ProgressModel.getAttendanceStats(
      student_id,
      academic_year_id
    );

    if (!hours) {
      return res.json(null);
    }

    const required = Number(hours.required_hours || 0);
    const completed = Number(hours.completed_hours || 0);

    const completionPercent =
      required > 0
        ? Math.min(Math.round((completed / required) * 100), 100)
        : 0;

    const requiredHoursCompleted =
      completed >= required - 0.25;

    const dailyLogsComplete =
      Number(dailyLogs.total || 0) > 0 &&
      Number(dailyLogs.approved || 0) === Number(dailyLogs.total || 0);

    const narrativesApproved =
      Number(narratives.total || 0) > 0 &&
      Number(narratives.approved || 0) === Number(narratives.total || 0);

    const coordinatorVerified =
      requiredHoursCompleted &&
      dailyLogsComplete &&
      narrativesApproved;

    return res.json({
      student_name: hours.student_name,
      department_name: hours.department_name,
      course_name: hours.course_name,
      coordinator_name: hours.coordinator_name,
      company_name: hours.company_name,

      required_hours: required,
      completed_hours: completed,
      remaining_hours: Math.max(required - completed, 0),

      completion_percent: completionPercent,

      attendance,
      dailyLogs,
      narratives,

      checklist: {
        requiredHoursCompleted,
        dailyLogsComplete,
        narrativesApproved,
        coordinatorVerified
      }
    });

  } catch (err) {
    console.error("Progress fetch by ID error:", err);
    return res.status(500).json({
      error: "Failed to fetch progress"
    });
  }
};